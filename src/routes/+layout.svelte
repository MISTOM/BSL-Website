<script lang="ts">
    ...
    const footer = {
        copyright: '© 2025 Brisk Solutions. All rights reserved.',
        section_one_title: 'Company',
        section_one_links: [
            { url: '/about', text: 'About Us' },
            { url: '/team', text: 'Our Team' },
            { url: '/careers', text: 'Careers' }
        ],
        section_two_title: 'Services',
        section_two_links: [
            { url: '/services/web-development', text: 'Web Development' },
            { url: '/services/mobile-apps', text: 'Mobile Apps' },
            { url: '/services/cloud-solutions', text: 'Cloud Solutions' }
        ],
        section_three_title: 'Resources',
        section_three_links: [
            { url: '/blog', text: 'Blog' },
...
<nav>
        ...
        <div class="flex items-center">
            ...
            {#if showMenu}
                <div class="fixed inset-0 z-50 h-full w-full text-white">
                    ...
                    >
                        <button onclick={() => (showMenu = false)} class="absolute top-0 left-0 mt-8 ml-6">
                            <svg
                                class="h-8 w-8"
                                fill="none"
                                stroke="currentColor"
                        ...
                        {#each navigationItems as item}
                            <a
                                href={item.url}
                                class="inline-block border-b-4 border-transparent hover:border-[#5069c3]"
                            >
                                {item.text}
                            </a>
                        {/each}
                    </div>
                    <div class="bg-opacity-20 absolute inset-0 h-full w-full bg-[#5069c3]"></div>
                </div>
            {/if}
        </div>
        ...
        <div class="flex items-center justify-center">
            <a href="/" class="text-2xl font-bold tracking-widest text-white uppercase">
                ...
                {#if settings.logo}
                    <img
                        src={settings.logo}
                        class="h-20 w-64 object-contain object-center"
    ...
    <div class="absolute top-0 left-0 h-24 w-full bg-[#5069c3]"></div>
</nav>
...
<footer class="bg-gray-900 text-white text-opacity-40 font-semibold uppercase text-xs tracking-widest bg-opacity-80 px-12">
    <div class="container mx-auto grid grid-cols-1 lg:grid-cols-4 gap-12 text-center lg:text-left py-24">
        ...
        <div>
            ...
            {#each footer.section_one_links as item}
